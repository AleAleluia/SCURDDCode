...
public int distanceTo( int xEnd, int yEnd){
	...
	public static void attackAction(Character actor, Character target, Attack atk)
	{
		...
		if(roll>=target.ac)
		{
			...
			/*for(;i<atk.getAtkDmg().length();i++)
			{
				if(atk.getAtkDmg().charAt(i)!='d')
				{
					parser= parser+atk.getAtkDmg().charAt(i);
				}
			}
			numberOfDice = Integer.parseInt(parser);
			parser = null;

			for(;i<atk.getAtkDmg().length();i++)
			{
				if(atk.getAtkDmg().charAt(i)!='+')
				{
					parser= parser+atk.getAtkDmg().charAt(i);
				}
			}
			typeOfDice = Integer.parseInt(parser);
			parser =null;
			...
			modifier = Integer.parseInt(parser);*/
	...
	public static void magicAction(Character actor, Character target, Spell spell)
	{
		...
		/*for(;i<spell.getEffect().length();i++)
		{
			if(spell.getEffect().charAt(i)!='d')
			{
				parser= parser+spell.getEffect().charAt(i);
			}
		}
		numberOfDice = Integer.parseInt(parser);
		parser = null;
